...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # print(type(first), type(rest)) 
            # <class 'str'> <class 'scheme_reader.Pair'>
        # eval rest
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    while isinstance(expressions, Pair) and expressions.second is not nil:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('argument values & formal parameters does not match.')
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            return self.fn(*python_args)    # star tansfer list or turple to arguments
        except:
            raise SchemeError('{0} has wrong number of arguments'.format(self.fn))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        # print(clause.second) # (5)
        formals, values = Pair(clause.first, formals), Pair(scheme_eval(clause.second.first, env), values)
