        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    # place's ant instance attribute is thr container Ant
                    self.ant = insect
                else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant and self.place.ant.blocks_path:
            return True
        return False
        # return self.place.ant is not None or self.place.ant.blocks_path == False
        # END Problem 7
...
class Ant(Insect):
    ...
    blocks_path = True
    container = False
    implemented = True # False  # Only implemented Ant classes should be instantiated
    food_cost = 0
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        curr = self.place
        while curr is not hive:
            if curr.bees:
                return random.choice(curr.bees) # random_or_none(self.place.bees)
            curr = curr.entrance
        return None
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        curr = self.place
        while curr is not hive and self.max_range >= 0:
            if curr.bees:
                return random.choice(curr.bees) # random_or_none(self.place.bees)
            curr = curr.entrance
            self.max_range -= 1
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.first:
            ...
            while curr:
                if curr.ant:
                    ants += [curr.ant]
                    if hasattr(ants[-1], 'container') and ants[-1].container:
                        ants += [ants[-1].ant]
                curr = curr.exit
            for a in ants:
                if a and a not in self.has_been_double:
                    # print(a, a.damage)
                    a.damage *= 2
                    self.has_been_double += [a]
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        if self.first and not self.armor:
            bees_win()
        # END Problem 13
