...
'''
    ...
    if not lst1:
        ...
        if lst1[0] < lst2[0]:
            lst += [lst1[0]] + merge(lst1[1:], lst2[:])
        else:
            lst += [lst2[0]] + merge(lst1[:], lst2[1:])
    return lst
'''
...
def put_piece(board, max_rows, column, player):
    ...
    for r in range(max_rows-1, -1, -1):
        if board[r][column] == '-':
            row, board = r, replace_elem(board, r, replace_elem(board[r], column, player)) 
            break
    return row, board   
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for col in range(max_cols):
        s += str(get_piece(board, row, col))
    if player * num_connect in s.strip():
        return True
    return False
