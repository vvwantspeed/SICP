...
def prune_leaves(t, vals):
    ...
    if is_leaf(t) and label(t) in vals:
        return None
    # bs = [prune_leaves(b, vals) for b in branches(t) if prune_leaves(b, vals)]
    bs = []
    for b in branches(t):
        temp = prune_leaves(b, vals)
        if temp:
            bs += [temp]
...
def add_trees(t1, t2):
    ...
    return tree(la, [add_trees(x, y) for x, y in zip(bs1, bs2)])