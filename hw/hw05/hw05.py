...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        la = label(t)
    return tree(la, [replace_leaf(b, old, new) for b in branches(t)])
...
def is_weight(w):
    ...
    "*** YOUR CODE HERE ***"
    return is_leaf(w)
    return is_tree(w) and type(size(w)) == int and len(w) == 1
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        l, r = sides(m)
        lt, rt = length(l)*total_weight(end(l)), length(r) * total_weight(end(r))
        if lt != rt:
            return False
        return False not in [balanced(end(s)) for s in sides(m)]
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    ny = interval(-upper_bound(y), -lower_bound(y))
    return add_interval(x, ny)
...
def quadratic(x, a, b, c):
    ...
    # def f(t):
    #     return a*t*t + b*t + c
    # lower, upper, extreme = f(l), f(u), f(e)
    extremum = -b / (2 * a)
    ...
    '''
    extrema = [map(f, (lower_bound(x), upper_bound(x), extremum))]
